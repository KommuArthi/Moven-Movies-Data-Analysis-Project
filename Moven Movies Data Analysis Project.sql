...
# Create a view that will show the film, its category and the actors name. Filter 
#the results for the actor whose first name = ‘Nick’.
...
SELECT * FROM morning_batch.athlete_events;
...
# List the films from the film table ranked by their length in ascending order. Include the film title and its length in the output.
select title, length,
rank() over(order by length asc) 
as Length_Rank from film;


# Calculate the dense rank of films from the film table based on their Replacement cost in descending order. Include the film title and Replacement cost in the output.
select title, replacement_cost,
dense_rank() over( order by replacement_cost desc) as Rank_Replacement_Cost 
from film;


# Determine the row number for each customer from the customer table when ordered by their first name in ascending order.
select first_name, last_name,
row_number() over( order by first_name asc) as RN
 from customer;


# each film in the film table, list the film title, release year, and the title of the film that comes after it in terms of release year.
select title, release_year,
lead(title) over (order by release_year) as Next_film 
 from film;


# Calculate the running total of rental amounts for payments from the payment table.  Display the payment date, amount, and the running total.
...
# Write a query to generate a value for the activity_type column which returns the string “Active” or “Inactive” depending 
#                             on the value of the customer.active column
# Table name: Customer
...
# Write a query to retrieve the number of rentals for each active customer. For inactive customers the result should be 0.
#                             Use case expression and correlated subquery.
# Table name: Customer
select 
    c.customer_id,
    c.first_name,
...
# List the titles of films and the corresponding first and last names of actors who have played roles in those films 
# using inner join on film,film_actor and actor tables.
# Table name: film
select film.title,actor.first_name,actor.last_name from film inner join film_actor on film.film_id=film_actor.film_id
inner join actor on actor.actor_id=film_actor.actor_id ;


#  List the film titles and their respective category name using an INNER JOIN between film and film_category and category tables.
#Table name: film
select film.title,category.name from film inner join film_category on film.film_id=film_category.film_id
inner join category on category.category_id=film_category.category_id;


# List the first names, last names, and store IDs of staff members using left join between staff and store tables
#Table name: staff
select staff.first_name,staff.last_name,store.store_id from staff left join store on staff.store_id=store.store_id;


# List the first and last names of customers, along with the titles of films they have rented and the corresponding rental dates using
# left join between customer ,rental ,inventory and film tables.
#Table name: film
